...
Input.prototype.validateAction = function (args) {
    try {
        ...
        if (args[action] === true) throw new Error(`please add add value to ${action}`);
...
Input.prototype.mapKey = function (key, args) {
    switch (key) {
        case "a":
            args.add = args["a"];
            delete args["a"];
