...
class Input {
  ...
  validateAction(args) {
    try {
      ...
      if ((args[action] === true || args[action] === '') && action !== 'list')
        throw new Error(`please add value to ${action}`);

      this.payload = args[action];
      this.category = args.category;
      // if user didnt provide a catigory
      if (this.category === true) throw new Error(`please add value to category`);
